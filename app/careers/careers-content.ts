/**
 * Content model for the marketing copy on the /careers page.
 *
 * MIRRORS energy-talent-cms/app/lib/content/careers-content.ts — the two must
 * stay in step. The defaults below are also the fallback rendered when the CMS
 * is unreachable, so the page can never come up blank.
 *
...
export const DEFAULT_CAREERS_CONTENT: CareersPageContent = {
  hero: {
    kicker: "BUILD WITH ENERGY TALENTS",
...
export async function getCareersContent(): Promise<CareersPageContent> {
  ...
  try {
    ...
    return withCareersDefaults(json.data?.content);
  } catch {
