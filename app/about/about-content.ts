 ...
 *
 * MIRRORS energy-talent-cms/app/lib/content/about-content.ts — keep the two in
 * step. The defaults are also the fallback rendered when the CMS is
 * unreachable, so /about can never come up blank.
 *
...
/** Fetch the About copy from the CMS, falling back to the shipped defaults. */
export async function getAboutContent(): Promise<AboutPageContent> {
  ...
  try {
    ...
    const json = (await res.json()) as {
      aboutPage?: { content?: unknown };
      data?: { content?: unknown };
    };
    const stored = json.aboutPage?.content ?? json.data?.content;
    return withAboutDefaults(stored);
  } catch {
